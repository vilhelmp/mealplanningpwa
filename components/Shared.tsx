...
export const Icons = {
  ...
  Recipes: ({ className }: { className?: string }) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M2 12h20"></path><path d="M7 12v-3a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v3"></path><path d="M12 9V5"></path><path d="M11 5h2"></path></svg>
  ),
  ...
  Star: ({ className, fill, key }: { className?: string, fill?: boolean, key?: any }) => (
    <svg key={key} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill={fill ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>
  ),
...
export const Button = ({ children, onClick, variant = 'primary', className = '', type = 'button', disabled = false }: any) => {
  ...
  const variants = {
    primary: "bg-nordic-primary text-white shadow-lg shadow-teal-900/10 hover:bg-teal-700",
    secondary: "bg-white text-nordic-secondary border border-gray-200 hover:bg-gray-50",
    ghost: "bg-transparent text-nordic-muted hover:bg-gray-100",
...
export const Card = ({ children, className = '', onClick, ...props }: any) => (
  <div onClick={onClick} className={`bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden ${className}`} {...props}>
    {children}
  </div>
);
...
export const Badge = ({ children, variant = 'default' }: any) => {
    const variants = {
        default: "bg-white/90 text-nordic-text backdrop-blur-sm",
        outline: "border border-gray-200 text-gray-500 bg-gray-50",
        accent: "bg-nordic-accent text-white"
    };
    return (
        <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide ${variants[variant as keyof typeof variants]}`}>
            {children}
        </span>
    );
};
...
export const Input = (props: any) => (
    <input 
        {...props}
        className={`w-full px-4 py-3 rounded-xl bg-gray-50 border-gray-200 border focus:bg-white focus:border-nordic-primary focus:ring-1 focus:ring-nordic-primary outline-none transition-all ${props.className || ''}`}
    />
);
